  ...
  this.initializeCheckout().then(() => {
    ...
    displayProductDetails(product) {
      ...
      product.variants.forEach((variant) => {
        availableSizes.push(variant.selectedOptions.find(option => option.name.toLowerCase() === 'size').value);
      });
    ...
    view3dModel(){
        let src = 'models/Sause Hoodie.glb';
    ...
    async addToCart(variantId) {
      try {
        ...
        this.updatedCheckout = await client.checkout.addLineItems(checkout.id, lineItemsToAdd);
    
        // Use SweetAlert instead of normal alert
        Swal.fire({
          title: "Success!",
          text: "Added " + String(document.getElementById("quantity").value) + " products to cart",
          icon: "success",
          button: "OK",
        });
      } catch (error) {
